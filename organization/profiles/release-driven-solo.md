...
- existing `docs/agent/`, public app/CMS/Rust boundaries, release workflow, and project quality/security docs remain project-specific authorities.
- existing custom Skills/adapters remain separate from upstream-managed project-init Skills.
- Bun is already part of the product toolchain, so `bunx skills` does not introduce a second package manager.
