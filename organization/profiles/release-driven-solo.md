...
my-web-2025 の current release-driven development で使用する Operating Model を定義します。
...
- GitHub Issues / Project が current durable work / planning surface。未採用の Linear をこの reconciliation だけで canonical 化しない。
- GitHub Issues が durable implementation / dependency facts の canonical authority
- planning surface は implementation/dependency SoT を置換しない
...
- 2025 edition の maintenance repository として既存 architecture / CMS / submodule / Cloudflare deployment contract を維持する。
- Bun / Rust の既存 canonical version sourcesを尊重し、mise へ競合 pin を作らない。
- SQLite content data と user-uncommitted data は mutable ownership / safety boundary を維持する。
