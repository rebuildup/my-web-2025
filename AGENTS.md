...
- **`external/<tool-slug>/` per-tool submodules** (Phase 1 + Phase 2-N, 14 tools): text-counter / color-palette / sequential-png-preview / svg2tsx / business-mail-block / code-type-p5 / fillgen / qr-generator / pomodoro / history-quiz / pi-game / ae-expression / prototype. `external/ui` is a placeholder for the Phase N+1 UI extraction. Bridges in `src/app/tools/<slug>/page.tsx` use `next/dynamic({ ssr: false })` → barrel import via 4-dot relative path. Tool-specific deps live in each submodule's `package.json`. **`transpilePackages` array in `next.config.ts`** must be extended per batch. tsconfig.json carries `@rebuildup/tool-<slug>` path aliases. Adding a new tool requires: helper scaffold + source move + bridge rewrite + path alias + transpilePackages entry.
- **Submodule branch workflow** (2026-08-29, P3-B): `develop` is the working branch in each of the 13 tool submodules (`external/ui` parked on `master`). New work lands on `develop` and merges into `main` via PR on the submodule repo. The parent repo refreshes gitlinks with `git submodule update --remote --merge` from the repo root (gitlink は origin/main tip に自動進行). Manual gitlink SHA bump は **禁止**. 13 entries have `.gitmodules` で `branch = main` 宣言済; `external/ui` remains on `master` (parked).
- **`external/ui` submodule default branch is `master`, not `main`** (Phase 1, parked as Phase N+2 cleanup): All 12 Phase 2-N tool submodules default to `main` (per Phase 1 R-1). `external/ui` was created in Phase 1 with `master` as the default. Renaming would require: `git branch -m master main` in `external/ui` + push `main` + delete `master` on remote + parent gitlink bump. Not blocking; Phase 2-N shipped without it.
