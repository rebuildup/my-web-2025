...
export const useAccessibility = () => {
	...
	const runAccessibilityChecks = useCallback(() => {
		...
		elements.forEach((element) => {
			...
			if (element.tagName.match(/^H[1-6]$/)) {
				...
				if (previousHeading) {
					const previousLevel = parseInt(previousHeading[0].charAt(1), 10);
	...
	const getColorBlindnessFilter = useCallback((type: string): string => {
		const filters = {
	...
	const simulateColorBlindness = useCallback(
		...
		[announce, getColorBlindnessFilter],
	);
...
const _meetsContrastRequirement = (
	color1: string,
	color2: string,
	level: "AA" | "AAA" = "AA",
): boolean => {
