 ...
 */
function _getRecentSearches(): SearchHistoryItem[] {
 ...
 */
function _getPopularSearches(): Array<{ query: string; count: number }> {
	const history = getSearchHistory();
 ...
 */
function _searchHistory(query: string): SearchHistoryItem[] {
	if (!query.trim()) {
		return [];
	}
