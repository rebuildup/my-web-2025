...
export class ResourcePreloader {
	...
	public static preloadResource(url: string, priority: number = 0): void {
		...
		ResourcePreloader.preloadQueue.push({ url, priority });
		ResourcePreloader.preloadQueue.sort((a, b) => b.priority - a.priority);

		// Process queue immediately
		ResourcePreloader.processPreloadQueue();
	}

	// Process preload queue
	private static processPreloadQueue(): void {
		while (ResourcePreloader.preloadQueue.length > 0) {
			const { url } = ResourcePreloader.preloadQueue.shift()!;

			// Skip if already preloaded
			if (ResourcePreloader.preloadedResources.has(url)) continue;
	...
	public static preloadCriticalResources(): void {
		...
		criticalResources.forEach(({ url, priority }) => {
			ResourcePreloader.preloadResource(url, priority);
		});

		// Ensure queue is processed
		ResourcePreloader.processQueue();
	}
}
...
export class MemoryOptimizer {
	...
	public static register(cleanup: () => void): void {
		MemoryOptimizer.disposables.add(cleanup);
	}

	// Unregister cleanup function
	public static unregister(cleanup: () => void): void {
		MemoryOptimizer.disposables.delete(cleanup);
	}

	// Clean up all registered disposables
	public static cleanup(): void {
		MemoryOptimizer.disposables.forEach((cleanup) => {
			try {
				cleanup();
			} catch (error) {
		...
		MemoryOptimizer.disposables.clear();
	}
