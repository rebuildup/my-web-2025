...
export class LCPOptimizer {
	...
	public static optimizeFontLoading(): void {
		// 既存のフォント読み込み（Google Fonts / Adobe Fonts）に任せる
		// 空の @font-face を注入するとブラウザがフォント解決に失敗するため何もしない
		return;
	}
...
export class CLSOptimizer {
	...
	public static optimizeFontLoading(): void {
		// ローカル @font-face (woff2) を持たないため preload は行わない
		// Google Fonts / Adobe Fonts に任せる
		return;
	}
}
