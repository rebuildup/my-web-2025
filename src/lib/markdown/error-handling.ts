...
import { EmbedError, EmbedReference, ValidationResult } from "@/types/content";
...
export class EmbedValidator {
  ...
  private validateImageEmbeds(
    ...
    lines.forEach((line, currentLineIndex) => {
      ...
      while ((match = this.embedPatterns.image.exec(line)) !== null) {
  ...
  private validateVideoEmbeds(
    ...
    lines.forEach((line, currentLineIndex) => {
      ...
      while ((match = this.embedPatterns.video.exec(line)) !== null) {
        const index = parseInt(match[1]);
        const column = match.index;
  ...
  private validateLinkEmbeds(
    ...
    lines.forEach((line, currentLineIndex) => {
      ...
      while ((match = this.embedPatterns.link.exec(line)) !== null) {
        const index = parseInt(match[1]);
        const column = match.index;
  ...
  private validateIframeEmbeds(content: string, warnings: string[]): void {
    ...
    lines.forEach((line, currentLineIndex) => {
      ...
      while ((match = this.embedPatterns.iframe.exec(line)) !== null) {
        const src = match[1];
