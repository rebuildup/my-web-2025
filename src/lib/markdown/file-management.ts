...
export class MarkdownFileManager {
		...
		try {
			...
			if (path.isAbsolute(filePath)) {
				...
				if (!filePath.includes(this.basePath)) {
					...
					// Check if the first segment is a valid content type directory
	...
	sanitizeContent(content: string): void {
		...
		for (const m of content.matchAll(iframeRegex)) {
		...
		// Check for null bytes and control characters without regex control ranges
