...
renderer.code = (code: string, language?: string) => {
  ...
  const className = validLanguage ? `language-${language}` : '';
  
  return `<pre><code class="hljs ${className}">${code}</code></pre>`;
};
...
renderer.image = (src: string, title: string | null, alt: string) => {
  const titleAttr = title ? ` title="${title}"` : '';
  return `<img src="${src}" alt="${alt}"${titleAttr} loading="lazy" class="responsive-image" />`;
};
 ...
 */
export async function processMarkdownFiles(files: { path: string; content: string }[]): Promise<{ [path: string]: ProcessedMarkdown }> {
 ...
 */
export function validateMarkdownMetadata(metadata: MarkdownMetadata): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
