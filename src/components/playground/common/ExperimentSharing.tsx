...
export function ExperimentSharing({
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-background border border-foreground max-w-md w-full mx-4 rounded-lg">
        {/* Header */}
        ...
        <div className="p-4 space-y-4">
          {/* Experiment Info */}
          <div className="bg-base border border-foreground p-3 rounded">
            <h4 className="font-medium text-primary">{experiment.title}</h4>
            <p className="text-sm text-foreground mt-1">
              {experiment.description}
            </p>
            <div className="flex flex-wrap gap-1 mt-2">
              <span className="text-xs border border-foreground px-2 py-1 rounded">
                {experiment.category}
              </span>
              <span className="text-xs border border-foreground px-2 py-1 rounded">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center space-x-2">
                <input
                  ...
                  value={shareURL}
                  readOnly
                  className="flex-1 border border-foreground bg-background text-foreground p-2 text-sm rounded"
                />
                <button
                  onClick={copyToClipboard}
                  className="flex items-center space-x-1 px-3 py-2 border border-foreground hover:border-accent hover:text-accent transition-colors rounded"
                >
                  {copied ? (
          ...
          {shareData && (
            <div className="bg-base border border-foreground p-3 rounded">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div>
                  <span className="text-foreground opacity-70">Quality:</span>
                ...
                <div>
                  <span className="text-foreground opacity-70">Device:</span>
                  <span className="ml-1 text-accent">
          ...
          <div className="flex items-center space-x-3">
            <button
              ...
              disabled={!shareURL}
              className="flex-1 flex items-center justify-center space-x-2 px-4 py-2 bg-accent text-background hover:bg-accent-dark transition-colors disabled:opacity-50 disabled:cursor-not-allowed rounded"
            >
            ...
            <button
              onClick={copyToClipboard}
              disabled={!shareURL}
              className="flex items-center space-x-2 px-4 py-2 border border-foreground hover:border-accent hover:text-accent transition-colors disabled:opacity-50 disabled:cursor-not-allowed rounded"
            >
          ...
          {/* Privacy Notice */}
          <div className="text-xs text-foreground opacity-70 bg-base border border-foreground p-2 rounded">
            <p>
  ...
  return (
    <>
      <button
        ...
        className={`flex items-center space-x-2 px-3 py-2 border border-foreground hover:border-accent hover:text-accent transition-colors ${className}`}
      >
        <Share2 className="w-4 h-4" />
        <span className="text-sm">Share</span>
      </button>
