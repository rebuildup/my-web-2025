...
function buildPreviewStyle(height: number): CSSProperties {
	return {
		width: "100%",
		height,
		borderRadius: 8,
...
export function SpacerBlock({
	...
	const previewStyle = buildPreviewStyle(height);
	...
	return (
		<section style={wrapperStyle} className="block-spacer">
			...
			<div className="spacer-controls" style={controlsStyle}>
				...
				<input
					id="spacer-lines-input"
					type="number"
					min={0}
					max={50}
					value={Number.isNaN(lines) ? 0 : lines}
					disabled={readOnly}
					aria-label="Spacer lines"
					onChange={(e) => {
						const v = Math.max(
							0,
