    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <label className="block noto-sans-jp-regular text-sm font-medium text-foreground">
            Categories
            {maxSelections && (
        ...
        {filteredCategories.map((category) => {
          ...
          return (
            <button
              ...
              className={`
                px-4 py-2 text-sm font-medium border transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-foreground focus:ring-offset-2 focus:ring-offset-background
                ${
                  isSelected
                    ? "bg-foreground text-background border-foreground"
                    : "bg-background text-foreground border-foreground hover:bg-foreground hover:bg-opacity-10"
                }
      ...
      {selectedCategories.length > 0 && (
        <div className="bg-base border border-foreground rounded-lg p-3">
          ...
          <div className="flex flex-wrap gap-2">
            {selectedCategories.map((category) => {
              ...
              return (
                <span
                  key={category}
                  className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-foreground text-background"
                >
                  {categoryInfo.label}
                  <button
                    type="button"
