...
export default function PortfolioInsights({
  ...
  const getChangeColor = (change: number, trend: string) => {
    if (trend === "stable") return "text-foreground";
  ...
  if (insights.length === 0) {
    return (
      <div className={`text-center py-8 ${className}`}>
        <div className="noto-sans-jp-regular text-sm text-foreground mb-2">
          インサイトがありません
        </div>
        <div className="noto-sans-jp-light text-xs text-foreground">
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="grid-system grid-1 xs:grid-2 sm:grid-2 gap-4">
        {insights.map((insight, index) => (
          <div
            key={index}
            className="bg-base border border-foreground p-4 hover:border-accent transition-colors"
          >
            ...
            <div className="mb-2">
              <span className="neue-haas-grotesk-display text-2xl text-primary">
                {insight.value.toLocaleString()}
              </span>
            </div>

            <p className="noto-sans-jp-light text-xs text-foreground mb-2">
              {insight.description}
            </p>
      ...
      <div className="bg-base border border-accent p-4">
        <h4 className="zen-kaku-gothic-new text-base text-primary mb-2">
          💡 おすすめアクション
        </h4>
