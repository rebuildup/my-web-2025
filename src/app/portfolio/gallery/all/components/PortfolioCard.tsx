...
export function PortfolioCard({
	...
	return (
		<article
			className="bg-base border border-main hover:border-accent transition-colors cursor-pointer group"
			onClick={onClick}
			onKeyDown={handleKeyDown}
			aria-label={`View details for ${item.title}`}
		>
			{/* Thumbnail */}
			<div className="aspect-video bg-base border-b border-main overflow-hidden relative">
				{item.thumbnail ? (
					<SafeImage
						src={item.thumbnail}
						alt={item.title}
			...
			<div className="p-4 space-y-3">
				...
				<div className="space-y-1">
					<h2 className="zen-kaku-gothic-new text-base text-main group-hover:text-accent transition-colors line-clamp-2">
						{item.title}
					</h2>
					<p className="noto-sans-jp-light text-xs text-main/70">
						{item.category}
					</p>
				</div>

				{/* Description - ONLY from item.description, NEVER from markdown content (Requirement 6.2) */}
				<p className="noto-sans-jp-light text-sm text-main text-truncate-2-lines">
					{item.description}
				</p>
				...
				<div className="tags-container flex flex-wrap gap-1">
					{((item as PortfolioContentItem).technologies || item.tags || [])
						...
						.map((tech, index) => (
							<span
								key={`${tech}-${index}`}
								className="noto-sans-jp-light text-xs text-main/80 border border-main/30 px-2 py-1 bg-base/50"
							>
								{tech}
							</span>
						))}
				...
				{/* Metadata */}
				<div className="flex items-center justify-between text-xs text-main/60">
