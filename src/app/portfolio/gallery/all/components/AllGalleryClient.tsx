...
export function AllGalleryClient({ initialItems }: AllGalleryClientProps) {
	...
	const [, _setSortOptions] = useState<SortOptions>({
	...
	const filteredAndSortedItems = useMemo(() => {
		...
		if (filter.technology && filter.technology !== "") {
			filtered = filtered.filter((item) =>
				item.tags?.includes(filter.technology),
			);
		}
		...
		<div data-testid="all-gallery-client">
			...
			<div className="mb-6 md:mb-8">
				<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
					...
					<div>
						<label
							htmlFor={technologySelectId}
							className="block text-sm font-medium mb-2"
						>
							Technology
						</label>
						<select
							id={technologySelectId}
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
					{filteredAndSortedItems.map((item) => (
						<Link
							...
							className="bg-base border border-main p-4 space-y-4 block hover:border-accent transition-colors"
