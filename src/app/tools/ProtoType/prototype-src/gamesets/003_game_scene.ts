...
import { loadcache_localranking, type RankingPlayer } from "./020_cacheControl";
...
export async function game_scene(app: PIXI.Application): Promise<void> {
	return new Promise(async (resolve) => {
		...
		function frame_anim(kpm: number) {
			...
			const offset = anim_max_width * ratio;
			const rect_alphas = kpm < 10 ? 0.4 : 0.8;
		...
		function transitionToResultScene() {
			...
			if (gameData.total_hit_cnt > 0) {
				...
				gameData.Score =
					tmp_kpm * (1 - gameData.Miss / gameData.total_hit_cnt) ** 3 * 100 +
			...
			while (gameData.CurrentSceneName === "game_scene") {
				...
				try {
					if (gameData.IsStarted) {
						const collectkeys = getNextKeysOptimized(
...
import type { Issue } from "./002_gameConfig";
