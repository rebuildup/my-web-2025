...
import { Download, X, Image, Film, FileImage } from "lucide-react";
import { FrameData, AnimationSettings, ExportSettings } from "../types";
  ...
  return (
    <div className="bg-base border border-foreground p-6 rounded-lg">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-lg font-semibold text-primary">エクスポート設定</h3>
        <button
          onClick={onClose}
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="grid grid-3 gap-3">
            ...
            >
              ...
              <p className="text-sm text-foreground">MP4動画として出力</p>
            </button>
          </div>
        </div>
        ...
        {/* Export Info */}
        <div className="p-4 bg-background border border-foreground/20 rounded-lg">
          <h4 className="font-medium mb-2 text-primary">エクスポート情報</h4>
          <div className="space-y-1 text-sm">
            <div className="flex justify-between">
              <span className="text-foreground/70">フレーム数:</span>
              <span>{frames.length}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-foreground/70">推定ファイルサイズ:</span>
              <span>{formatFileSize(estimatedFileSize())}</span>
            </div>
            {exportSettings.format !== "frames" && (
              <>
                <div className="flex justify-between">
                  <span className="text-foreground/70">出力解像度:</span>
                  <span>
                    {exportSettings.width} × {exportSettings.height}px
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-foreground/70">フレームレート:</span>
