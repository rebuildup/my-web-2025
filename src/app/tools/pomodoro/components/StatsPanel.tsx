...
import { X, Clock, Target, Zap, Calendar } from "lucide-react";
import { PomodoroStats, PomodoroSession } from "../types";
  ...
  return (
    <div className="bg-base border border-foreground p-6 rounded-lg">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-lg font-semibold text-primary">統計情報</h3>
        <button
          onClick={onClose}
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-2 gap-4">
          <div className="bg-background border border-foreground/20 p-4 rounded-lg text-center">
        ...
        <div className="space-y-4">
          <h4 className="font-medium text-primary">詳細統計</h4>

          <div className="grid grid-2 gap-4 text-sm">
            <div className="flex justify-between">
              <span className="text-foreground">総セッション数:</span>
              <span className="font-medium">{stats.totalSessions}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-foreground">総休憩時間:</span>
              <span className="font-medium">
                {formatTime(stats.totalBreakTime)}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-foreground">最長連続記録:</span>
              <span className="font-medium">{stats.longestStreak}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-foreground">今週のセッション:</span>
        ...
        {/* Recent Sessions */}
        <div className="space-y-4">
          <h4 className="font-medium text-primary">最近のセッション</h4>
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {getRecentSessions().length > 0 ? (
              getRecentSessions().map((session) => (
                <div
                  key={session.id}
                  className="flex items-center justify-between p-3 bg-background border border-foreground/20 rounded"
                >
        ...
        {stats.completedPomodoros > 0 && (
          <div className="space-y-4">
            <h4 className="font-medium text-primary">生産性の洞察</h4>

            <div className="bg-background border border-foreground/20 p-4 rounded-lg">
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  <span className="text-foreground">作業効率:</span>
                  <span className="font-medium">
                    {Math.round(
                      (stats.totalWorkTime /
                ...
                <div className="flex justify-between">
                  <span className="text-foreground">1日平均ポモドーロ:</span>
                  <span className="font-medium">
                    {Math.round(
                      stats.completedPomodoros /
