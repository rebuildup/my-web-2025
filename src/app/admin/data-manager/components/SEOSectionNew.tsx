	...
	const handleKeywordsChange = (keywordsString: string) => {
	 ...
	 return (
	   <div className="space-y-4">
	     <h3 className="neue-haas-grotesk-display text-xl text-main leading-snug">SEO Settings</h3>
	     {/* Basic SEO */}
	     <div className="space-y-4">
	       <div>
	     ...
	     {/* Social Media Images */}
	     <div className="space-y-4">
	       <h4 className="noto-sans-jp-regular text-sm font-medium text-main">Social Media Images</h4>
	       <div>
	     ...
	     {/* Advanced Settings */}
	     <div className="space-y-4">
	       <h4 className="noto-sans-jp-regular text-sm font-medium text-main">Advanced Settings</h4>
	       <div>
	     ...
	     {(seo?.title || seo?.description) && (
	       <div className="border border-gray-200 p-4 rounded">
	         <h4 className="noto-sans-jp-regular text-sm font-medium text-main mb-3">Search Engine Preview</h4>
	         <div className="space-y-2">
	           <div className="text-blue-600 text-lg hover:underline cursor-pointer">
	             {seo?.title || "Page Title"}
	           </div>
	           <div className="text-green-700 text-sm">https://yusuke-kim.com/example-url</div>
	           <div className="text-gray-600 text-sm">{seo?.description || "Page description will appear here..."}</div>
	         </div>
	       </div>
	     )}
	     ...
	     {(seo?.title || seo?.description || seo?.ogImage) && (
	       <div className="border border-gray-200 p-4 rounded">
	         <h4 className="noto-sans-jp-regular text-sm font-medium text-main mb-3">Social Media Preview</h4>
	         <div className="border border-gray-300 rounded overflow-hidden max-w-md">
	           ...
	           <div className="p-3 bg-white">
	             <div className="text-sm font-medium text-gray-900 mb-1">{seo?.title || "Page Title"}</div>
	             <div className="text-xs text-gray-600 mb-1">{seo?.description || "Page description will appear here..."}</div>
	     ...
	     <div className="border border-gray-200 p-4 rounded bg-gray-50">
	       <h4 className="noto-sans-jp-regular text-sm font-medium text-main mb-3">Dynamic Generated Mockup</h4>
	       <p className="text-xs text-gray-500 mb-2">Based on current title, category, tags, and thumbnail.</p>
	       <div className="border border-gray-300 rounded overflow-hidden max-w-md">
	         <div className="aspect-video bg-gray-100">
	           {/* eslint-disable-next-line @next/next/no-img-element */}
	           <img
	             src={`/api/og?title=${encodeURIComponent(title || "Untitled")}&category=${encodeURIComponent(category || "Portfolio")}&tags=${encodeURIComponent((tags || []).join(","))}&thumbnail=${encodeURIComponent(thumbnail || "")}`}
