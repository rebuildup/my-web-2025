...
export default function AdminAnalyticsPage() {
	...
	const CardStyle =
		"bg-base border border-main p-4 space-y-4 focus:outline-none focus:ring-2 focus:ring-main focus:ring-offset-2 focus:ring-offset-base";
	const Card_title =
					...
					<div className="container-system">
						<div className="space-y-10">
							...
							<button type="button"
								...
								className="border border-main px-4 py-2 hover:bg-main hover:text-base transition-colors"
							>
								Retry
							</button>
						</div>
						...
						<header className="space-y-6">
							...
							<div className="flex gap-4">
								...
								<button type="button"
									...
									className="border border-accent px-4 py-2 hover:bg-accent hover:text-main transition-colors disabled:opacity-50"
								>
							...
							>
								...
								<div className="grid-system grid-1 md:grid-2 gap-4">
									{data.health.checks.map((check) => (
										<div
											key={`${check.name}-${check.timestamp}`}
											className="border border-main p-3 space-y-2"
										>
											<div className="flex items-center justify-between">
								...
								{data.health.alerts.length > 0 && (
									<div className="space-y-2">
										...
										{data.health.alerts.map((alert) => (
											<div
												key={`${alert.level}-${alert.timestamp}-${alert.message}`}
									...
									<div className="grid-system grid-2 md:grid-4 gap-2">
										{Object.entries(data.statistics.content.byStatus).map(
											([status, count]) => (
												<div
													key={status}
													className="border border-main p-2 text-center"
												>
							...
							<div className={CardStyle}>
								...
								<div className="space-y-2">
									{data.metrics.slice(0, 10).map((metric) => (
										<div
											key={metric.id}
											className="border border-main p-3 flex items-center justify-between"
										>
