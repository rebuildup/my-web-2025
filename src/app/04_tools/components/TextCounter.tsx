...
const TextCounter: React.FC = () => {
  ...
  const seoAnalysis = useMemo(() => {
    ...
    return {
      ...
      titleLength,
      titleMessage: titleLength < 30 ? 'Too short for SEO title' : titleLength > 60 ? 'Too long for SEO title' : 'Good length for SEO title',
      metaOptimal: metaDescLength >= 120 && metaDescLength <= 160,
      metaDescLength,
      metaMessage: metaDescLength < 120 ? 'Too short for meta description' : metaDescLength > 160 ? 'Too long for meta description' : 'Good length for meta description',
  ...
  return (
    <div className="space-y-6">
      <div className="border-foreground/20 border p-6">
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            <button onClick={() => setText('')} className="border-foreground/20 text-foreground/70 hover:border-primary/50 border px-4 py-2 text-sm transition-colors">
              Clear
            </button>
            <button onClick={() => copyToClipboard(text)} className="border-primary text-primary hover:bg-primary/10 flex items-center space-x-1 border px-4 py-2 text-sm transition-colors">
              <Copy size={14} />
              <span>Copy Text</span>
            </button>
          </div>
      ...
      <div className="border-foreground/20 border p-6">
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-7">
          <div className="border-foreground/20 bg-gray/50 border p-3 text-center">
            <div className="neue-haas-grotesk-display text-primary mb-1 text-2xl">{textStats.characters.toLocaleString()}</div>
            <div className="noto-sans-jp text-foreground/70 text-xs">Characters</div>
          </div>
          <div className="border-foreground/20 bg-gray/50 border p-3 text-center">
            <div className="neue-haas-grotesk-display text-primary mb-1 text-2xl">{textStats.words.toLocaleString()}</div>
            <div className="noto-sans-jp text-foreground/70 text-xs">Words</div>
          </div>
          <div className="border-foreground/20 bg-gray/50 border p-3 text-center">
            <div className="neue-haas-grotesk-display text-primary mb-1 text-2xl">{textStats.sentences}</div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="border-foreground/20 border p-6">
          <div className="mb-4 flex items-center justify-between">
            ...
            <button onClick={() => setShowSEO(!showSEO)} className={`text-sm transition-colors ${showSEO ? 'text-primary' : 'text-foreground/60'}`}>
              {showSEO ? 'Hide' : 'Show'}
            </button>
          </div>
          {showSEO && (
            <div className="space-y-4">
              <div className="border-foreground/20 border p-3">
                <span className="noto-sans-jp text-foreground text-sm font-medium">Readability Score: </span>
                <span className={`text-sm font-medium ${readability.color}`}>{seoAnalysis.readabilityScore}/100 ({readability.level})</span>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="border-foreground/20 border p-6">
        ...
        <div className="flex flex-wrap gap-2">
          <button onClick={() => exportAnalysis('txt')} className="bg-primary hover:bg-primary/80 flex items-center space-x-2 px-4 py-2 text-white transition-colors">
            <Download size={16} />
