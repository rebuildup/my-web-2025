...
Next.js 16 (App Router, standalone) + Bun + PM2 で動く現行ポートフォリオサイトの技術ドキュメント.データはリポジトリ内の `data/contents/*.db` をビルド時にスタンドアロン成果物へ同梱し、外部DBは不要.
...
- Playground / Tools（ProtoType 同梱、`bun proto:sync` で更新）
...
- 言語/ツール: TypeScript 5.9, bun, Biome (lint/format)
